    ...
    stages {
        ...
        stage('Verify Packer Windows Dependant AMIS') {
         ...
         //       stage('Verify Windows Server Jenkins Slave') { steps { script {verify_win_image('windows_slave.json')}}}
                ...
                stage('Verify Windows Server MIS Nart') { steps { script {verify_win_image('windows_misnart_bcs.json')}}}
                stage('Verify Windows Server MIS Nart') { steps { script {verify_win_image('windows_misnart_bfs.json')}}}
            }
        }
